...
def donchian_channel(highs, lows, period=50):
    """
    Donchian Channel.
    Son `period` mumun en yüksek ve en düşük değerini döner.
    Mevcut mum dahil edilmez (kapanmış mumlardan hesaplanır).
    
    Returns: (upper, lower) — float, float
    """
    if len(highs) < period + 1:
        return None, None
    
    # Son kapanmış period kadar mum (en güncel açık mum hariç)
